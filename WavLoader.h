...
class PosixFileWrapper : public FileWrapper {
 ...
 PosixFileWrapper(std::string fileName)
   : FileWrapper(fileName), _mode("rw") {
    
  }

 PosixFileWrapper(std::string fileName, const char* mode)
   : FileWrapper(fileName), _mode(mode) {
...
class SDFileWrapper : public FileWrapper {
 public:
  ...
  virtual bool    open() {
    // We do not hold an open file-descriptor
    // because the SD library only allows
    // one open file at a time.
    // Hopefully this does not suck for performance.
